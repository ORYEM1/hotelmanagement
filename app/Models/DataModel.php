...
class DataModel extends Model
{
    ...
    public function get_users($parameters=array())
    {
        ...
        $params['searchable_fields']=array('first_name','other_names','username','phone_number');
        $params['fields']=array('id','first_name','other_names','phone_number','user_roles.role','status');
        ...
        {
            $row=array($record['id'],$record['first_name'],$record['other_names'],$record['role'],$record['phone_number'],$statuses[$record['status']]??$record['status']);
    ...
    public function get_countries($parameters=array())
    {
        $params=array();
        $params['query_params']=$parameters;
        $params['main_table']='countries';
    ...
    public function get_apps($parameters=array())
    {
        $params=array();
        $params['query_params']=$parameters;
        $params['main_table']='apps';
        $params['fields']=array('id', 'timestamp', 'app_name', 'login_name', 'whitelist_ip', 'status');
        $params['searchable_fields']=array('app_name');
        $query=$this->get_data($params);
        if(isset($parameters['download_data'])&&$parameters['download_data'])
        {
            return $query['db_data'];
        ...
        foreach($data as $record)
        {
            $row=array($record['id'],$record['timestamp'],$record['app_name'],$record['login_name'],$statuses[$record['whitelist_ip']]??$record['whitelist_ip'],$statuses[$record['status']]??$record['status']);
            $url="/apps/view_app/{$record['id']}";
            $row[]="<a href='{$url}'  title='View' class='open_modal'><i class='fa fa-search'></i></a>";

            $url="/apps/edit_app/{$record['id']}";
            $row[]="<a href='{$url}'  title='Edit' class='open_modal'><i class='fa fa-edit'></i></a>";

            $url="/apps/reset_password/{$record['id']}";
            $row[]="<a href='{$url}'  title='Reset Password' class='open_modal'><i class='fa fa-unlock'></i></a>";
    ...
    public function get_transaction_parameters($parameters=array())
    {
        ...
        $params['fields']=array('id','name','value', 'library_id', 'wallet_libraries.readable_name');
